...
<div x-data="{ open: false, type: 'qris' }" class="mb-6">
    ...
    <div x-show="open" x-cloak class="fixed inset-0 z-50 flex items-center justify-center px-4">
        ...
        <div x-show="open" x-transition class="relative bg-white w-full max-w-md rounded-3xl p-6 shadow-2xl max-h-[90vh] overflow-y-auto">
            ...
            <form action="<?php echo e(route('owner.payment-settings.store')); ?>" method="POST" enctype="multipart/form-data" class="space-y-4">
                <?php echo csrf_field(); ?>
                <div>
...
<div class="grid md:grid-cols-2 gap-4">
    <?php $__empty_1 = true; $__currentLoopData = $paymentSettings; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $ps): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
        <div class="card-pop bg-white rounded-2xl border border-coffee-100 shadow-sm p-5 flex items-start justify-between gap-4">
            <div class="flex gap-3">
                <span class="text-3xl"><?php echo e($ps->icon()); ?></span>
                <div>
                    <div class="flex items-center gap-2">
                        <p class="font-bold text-coffee-800"><?php echo e($ps->displayLabel()); ?></p>
                        <span class="text-[10px] font-semibold px-2 py-0.5 rounded-full <?php echo e($ps->isCash() ? 'bg-orange-100 text-orange-700' : 'bg-indigo-100 text-indigo-700'); ?>">
                            <?php echo e($ps->isCash() ? 'TUNAI' : 'NON-TUNAI'); ?>

                        </span>
                    </div>
                    <?php if($ps->type === 'bank_transfer'): ?>
                        <p class="text-sm text-coffee-500 mt-1"><?php echo e($ps->account_number); ?> a.n. <?php echo e($ps->account_holder); ?></p>
                    <?php elseif($ps->type === 'cash'): ?>
                        <p class="text-sm text-coffee-500 mt-1"><?php echo e($ps->instruction); ?></p>
                    <?php elseif($ps->qris_image): ?>
                        <img src="<?php echo e(asset('storage/'.$ps->qris_image)); ?>" class="w-16 h-16 object-contain mt-2 rounded-lg border border-coffee-100">
                    <?php endif; ?>
                    <span class="inline-block mt-2 text-[11px] font-semibold px-2.5 py-0.5 rounded-full <?php echo e($ps->is_active ? 'bg-emerald-100 text-emerald-700' : 'bg-gray-100 text-gray-500'); ?>">
                        <?php echo e($ps->is_active ? 'Aktif' : 'Nonaktif'); ?>

                    </span>
                </div>
            </div>
            <div class="flex flex-col gap-2 shrink-0">
                <form action="<?php echo e(route('owner.payment-settings.toggle', $ps)); ?>" method="POST">
                    ...
                    <button class="btn-press text-xs font-semibold px-3 py-1.5 rounded-lg border border-coffee-200 text-coffee-600 hover:bg-coffee-50">
                        <?php echo e($ps->is_active ? 'Nonaktifkan' : 'Aktifkan'); ?>

                    </button>
                </form>
                <form action="<?php echo e(route('owner.payment-settings.destroy', $ps)); ?>" method="POST" onsubmit="return confirm('Hapus metode ini?')">
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\laragon\www\lila\resources\views/owner/payment-settings/index.blade.php ENDPATH**/ ?>