...
<head>
    ...
    <title><?php echo e($title ?? 'Dashboard'); ?> — Warkop Samalila</title>
        ...
        <aside :class="sidebarOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'"
            ...
            <div class="px-6 py-5 flex items-center gap-2 border-b border-coffee-700">
                ...
                <div>
                    ...
                    <p class="text-xs text-coffee-200 capitalize"><?php echo e(auth()->user()->role ?? ''); ?></p>
                </div>
            </div>
            ...
            <nav class="flex-1 px-3 py-4 space-y-1 overflow-y-auto">
                ...
                <?php if($role === 'owner'): ?>
                    ...
                    ?>
                    <?php $__currentLoopData = $ownerLinks; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $link): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <a href="<?php echo e(route($link['route'])); ?>"
                           class="nav-link flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm
                           <?php echo e(request()->routeIs($link['route']) || (str_starts_with($link['route'], 'owner.orders') && request()->routeIs('owner.orders.*')) ? 'bg-coffee-600 text-white font-semibold' : 'text-coffee-100 hover:bg-coffee-700'); ?>">
                            <span><?php echo e($link['icon']); ?></span> <?php echo e($link['label']); ?>

                        </a>
                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                <?php elseif($role === 'dapur'): ?>
                    <a href="<?php echo e(route('dapur.dashboard')); ?>" class="nav-link flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm bg-coffee-600 text-white font-semibold">
                        <span>🍳</span> Pesanan Masuk
                    </a>
                <?php elseif($role === 'superadmin'): ?>
                    <a href="<?php echo e(route('superadmin.dashboard')); ?>" class="nav-link flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm <?php echo e(request()->routeIs('superadmin.dashboard') ? 'bg-coffee-600 text-white font-semibold' : 'text-coffee-100 hover:bg-coffee-700'); ?>">
                        <span>📊</span> Dashboard
                    </a>
                    <a href="<?php echo e(route('superadmin.accounts.index')); ?>" class="nav-link flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm <?php echo e(request()->routeIs('superadmin.accounts.*') ? 'bg-coffee-600 text-white font-semibold' : 'text-coffee-100 hover:bg-coffee-700'); ?>">
                        <span>👥</span> Kelola Akun
                    </a>
                    <a href="<?php echo e(route('superadmin.logs.index')); ?>" class="nav-link flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm <?php echo e(request()->routeIs('superadmin.logs.*') ? 'bg-coffee-600 text-white font-semibold' : 'text-coffee-100 hover:bg-coffee-700'); ?>">
        ...
        <div class="flex-1 flex flex-col min-w-0">
            <header class="bg-white border-b border-coffee-100 px-4 lg:px-8 py-4 flex items-center justify-between sticky top-0 z-20">
                <div class="flex items-center gap-3">
                    ...
                    <h1 class="text-lg lg:text-xl font-bold text-coffee-800"><?php echo e($title ?? 'Dashboard'); ?></h1>
                </div>
                <div class="text-xs sm:text-sm text-coffee-600 flex items-center gap-3">
                    ...
                    <span>Halo, <span class="font-semibold text-coffee-800"><?php echo e(auth()->user()->name ?? ''); ?></span> 👋</span>
                </div>
            </header>
            ...
            <main class="flex-1 p-4 lg:p-8">
                ...
                <?php if(session('wa_dapur_link')): ?>
                    <div class="mb-6 rounded-xl bg-blue-50 border border-blue-200 text-blue-700 px-4 py-3 flex items-center justify-between flex-wrap gap-2">
                        ...
                        <a href="<?php echo e(session('wa_dapur_link')); ?>" target="_blank"
...
<?php /**PATH C:\laragon\www\lila\resources\views/layouts/admin.blade.php ENDPATH**/ ?>