...
<div x-data="orderApp()" x-init="init()" x-cloak class="pb-28">
        ...
        <div class="max-w-7xl mx-auto space-y-3">
            ...
            <div class="flex gap-2 overflow-x-auto pb-1 no-scrollbar justify-start sm:justify-center">
                <?php $__currentLoopData = $categories; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $i => $category): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <button @click="activeCategory = '<?php echo e($category->id); ?>'"
                            :class="activeCategory === '<?php echo e($category->id); ?>' && searchQuery === '' ? 'bg-coffee-700 text-white shadow-lg scale-105' : 'bg-white text-coffee-700 border border-coffee-200'"
                        ...
                        <span><?php echo e($category->icon); ?></span> <?php echo e($category->name); ?>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-4 space-y-8">
        <?php $__empty_1 = true; $__currentLoopData = $categories; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $category): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
            <section x-show="hasCategoryMatches('<?php echo e($category->id); ?>')" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-2" x-transition:enter-end="opacity-100 translate-y-0">
                <h2 class="font-bold text-coffee-800 text-lg sm:text-xl mb-4 flex items-center gap-2"><?php echo e($category->icon); ?> <?php echo e($category->name); ?></h2>
                <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-3.5 sm:gap-5">
                    <?php $__currentLoopData = $category->menus; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $menu): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <div x-show="matchesSearch('<?php echo e(addslashes($menu->name)); ?>', '<?php echo e($category->id); ?>')" class="card-hover bg-white rounded-2xl overflow-hidden border border-coffee-100 shadow-sm flex flex-col justify-between">
                            <div>
                                <div class="h-28 sm:h-36 bg-coffee-100 overflow-hidden">
                                    <img src="<?php echo e($menu->photo_url); ?>" alt="<?php echo e($menu->name); ?>" class="w-full h-full object-cover" loading="lazy">
                                </div>
                                <div class="p-3.5">
                                    <p class="font-semibold text-sm text-coffee-800 leading-snug line-clamp-2 h-9"><?php echo e($menu->name); ?></p>
                                    <p class="text-coffee-500 font-bold text-sm mt-1">Rp <?php echo e(number_format($menu->price, 0, ',', '.')); ?></p>
                                </div>
                            </div>
                            <div class="p-3.5 pt-0">

                                <template x-if="!cartHas(<?php echo e($menu->id); ?>)">
                                    <button @click="addToCart(<?php echo e($menu->id); ?>, '<?php echo e(addslashes($menu->name)); ?>', <?php echo e($menu->price); ?>)"
                                ...
                                <template x-if="cartHas(<?php echo e($menu->id); ?>)">
                                    <div class="mt-2 flex items-center justify-between bg-coffee-50 rounded-lg px-1.5 py-1">
                                        <button @click="decrement(<?php echo e($menu->id); ?>)" class="btn-press w-7 h-7 rounded-md bg-white shadow text-coffee-700 font-bold">−</button>
                                        <span x-text="qtyOf(<?php echo e($menu->id); ?>)" class="font-bold text-sm text-coffee-800"></span>
                                        <button @click="increment(<?php echo e($menu->id); ?>)" class="btn-press w-7 h-7 rounded-md bg-coffee-700 text-white shadow font-bold">+</button>
                                    </div>
                                </template>
                            </div>
                        </div>
    ...
    <div x-show="statusOpen" x-cloak class="fixed inset-0 z-50 flex items-center justify-center px-4">
        ...
        <div x-show="statusOpen" x-transition class="relative bg-white w-full max-w-sm rounded-3xl p-6 shadow-2xl">
            ...
            <form action="<?php echo e(route('order.status')); ?>" method="GET" class="space-y-3">
    ...
    <form id="checkoutForm" method="POST" action="<?php echo e(route('order.store')); ?>" class="hidden">
        <?php echo csrf_field(); ?>
...
function orderApp() {
    return {
        categories: <?php echo json_encode($categories->map(fn($c) => ['id' => (string) $c->id]), 15, 512) ?>,
        allMenus: <?php echo json_encode($categories->flatMap(fn($c) => $c->menus->map(fn($m) => ['id' => $m->id, 'name' => $m->name, 'cat' => (string) $c->id]))) ?>,
        cart: [],
        activeCategory: '<?php echo e($categories->first()->id ?? ''); ?>',
        searchQuery: '',
        cartOpen: false,
        identityOpen: false,
        statusOpen: false,
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\laragon\www\lila\resources\views/customer/menu.blade.php ENDPATH**/ ?>