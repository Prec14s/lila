    ...
    <div class="overflow-x-auto">
        <table class="w-full text-sm whitespace-nowrap">
            ...
            <tbody class="divide-y divide-coffee-50 whitespace-nowrap">
                        ...
                        <td class="px-5 py-3 text-right whitespace-nowrap">
                            @if($order->payment_status === 'rejected')
                                <form action="{{ route('owner.orders.destroy', $order) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus pesanan ditolak ini?')" class="inline-block">
                                    @csrf
                                    @method('DELETE')
                                    <button class="btn-press text-xs font-semibold px-2.5 py-1 rounded-lg bg-red-100 text-red-700 hover:bg-red-200 transition border border-red-200 whitespace-nowrap">
                                        🗑️ Hapus
                                    </button>
                                </form>
                            @else
                                <a href="{{ route('owner.orders.receipt', $order) }}" target="_blank" class="btn-press text-xs font-semibold px-2.5 py-1 rounded-lg border border-coffee-200 text-coffee-600 hover:bg-coffee-50 whitespace-nowrap">
                                    🧾 Struk
                                </a>
                            @endif
                        </td>
                ...
                @empty
                    <tr><td colspan="8" class="px-5 py-10 text-center text-coffee-400 whitespace-nowrap">Belum ada pesanan.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
